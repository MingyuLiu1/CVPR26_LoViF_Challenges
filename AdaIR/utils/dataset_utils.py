...
class BioIRTrainDataset(Dataset):
    def __init__(self, args):
        ...
        self.de_dict = {'denoise_15': 0, 'denoise_25': 1, 'denoise_50': 2, 'derain': 3, 'dehaze': 4, 'deblur' : 5, 'enhance' : 6, 'desnow' : 7}
        # self.de_dict = {'desnow': 0, 'derain': 3, 'dehaze': 4, 'deblur' : 5, 'enhance' : 6}
    ...
    def _init_rs_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.derain_dir, 'LQ/'))
        temp_ids = image_list
        self.rs_ids= [{"clean_id" : x,"de_type":3} for x in temp_ids]
        # self.rain_ids = self.rain_ids * 20
        self.num_rain = len(self.rs_ids)
        print('Total rain Ids : {}'.format(self.num_rain))
        
    def _init_hazy_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.dehaze_dir, 'LQ/'))
        temp_ids = image_list
        self.hazy_ids= [{"clean_id" : x,"de_type":4} for x in temp_ids]
        # self.hazy_ids = self.hazy_ids * 20
        self.num_hazy = len(self.hazy_ids)
        print('Total hazy Ids : {}'.format(self.num_hazy))
        
    def _init_deblur_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.deblur_dir, 'LQ/'))
        temp_ids = image_list
    ...
    def _init_enhance_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.enhance_dir, 'LQ/'))
        temp_ids = image_list
    ...
    def _init_desnow_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.desnow_dir, 'LQ/'))
        temp_ids = image_list
        self.desnow_ids= [{"clean_id" : x,"de_type":7} for x in temp_ids]
        # self.desnow_ids = self.desnow_ids * 20
        self.num_desnow = len(self.desnow_ids)
        print('Total desnow Ids : {}'.format(self.num_desnow))
    ...
    def _get_derain_name(self, derain_name):
        gt_name = derain_name.replace("LQ", "GT")
        return gt_name
    
    def _get_dehaze_name(self, dehaze_name):
        gt_name = dehaze_name.replace("LQ", "GT")
        return gt_name

    def _get_deblur_name(self, deblur_name):
        gt_name = deblur_name.replace("LQ", "GT")
        return gt_name
    
    def _get_enhance_name(self, enhance_name):
        gt_name = enhance_name.replace("LQ", "GT")
        return gt_name
    
    def _get_desnow_name(self, desnow_name):
        gt_name = desnow_name.replace("LQ", "GT")
        return gt_name
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.derain_dir, 'LQ/', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.derain_dir, 'GT/', sample["clean_id"])).convert('RGB')), base=16)
                clean_name = self._get_derain_name(sample["clean_id"])
            elif de_id == 4:
                # Dehazing with SOTS outdoor training set
                # degrad_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('RGB')), base=16)
                # clean_name = self._get_nonhazy_name(sample["clean_id"])
                # clean_img = crop_img(np.array(Image.open(clean_name).convert('RGB')), base=16)
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.dehaze_dir, 'LQ/', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.dehaze_dir, 'GT/', sample["clean_id"])).convert('RGB')), base=16)
                clean_name = self._get_dehaze_name(sample["clean_id"])
            elif de_id == 5:
                # Deblur with deblur set
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.deblur_dir, 'LQ/', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.deblur_dir, 'GT/', sample["clean_id"])).convert('RGB')), base=16)
                clean_name = self._get_deblur_name(sample["clean_id"])
            elif de_id == 6:
                # Enhancement with LOL training set
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.enhance_dir, 'LQ/', sample["clean_id"])).convert('RGB')), base=16)
                ...
                # Enhancement with LOL training set
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.desnow_dir, 'LQ/', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.desnow_dir, 'GT/', sample["clean_id"])).convert('RGB')), base=16)
                clean_name = self._get_desnow_name(sample["clean_id"])
...
class DerainDehazeDataset(Dataset):
    def __init__(self, args, task="derain",addnoise = False,sigma = None):
        ...
        self.task_dict = {'derain': 0, 'dehaze': 1, 'deblur': 2, 'enhance': 3, 'desnow': 4, 'aio': 5}
        self.toTensor = ToTensor()
    ...
    def _init_input_ids(self):
        if self.task_idx == 0:
            ...
            self.ids = []
            name_list = os.listdir(self.args.deblur_path +'LQ/')
            self.ids += [self.args.deblur_path + 'LQ/' + id_ for id_ in name_list]
        elif self.task_idx == 3:
            self.ids = []
            name_list = os.listdir(self.args.enhance_path + 'LQ/')
            self.ids += [self.args.enhance_path + 'LQ/' + id_ for id_ in name_list]
        elif self.task_idx == 4:
            self.ids = []
            name_list = os.listdir(self.args.desnow_path + 'LQ/')
            self.ids += [self.args.desnow_path + 'LQ/' + id_ for id_ in name_list]
        elif self.task_idx == 5:
            self.ids = []
            name_list = os.listdir(self.args.aio_path + 'LQ/')
            self.ids += [self.args.aio_path + 'LQ/' + id_ for id_ in name_list]

        self.length = len(self.ids)

    def _get_gt_path(self, degraded_name):
        if self.task_idx == 0:
            gt_name = degraded_name.replace("LQ/", "GT/")
        elif self.task_idx == 1:
            dir_name = degraded_name.split("LQ/")[0] + 'GT/'
            # name = degraded_name.split('/')[-1].split('_')[0] + '.png'
            name = degraded_name.split('/')[-1].split('_')[0] #+ '.jpg'
