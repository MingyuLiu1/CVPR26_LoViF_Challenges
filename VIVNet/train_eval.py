...
class BioIRModel(pl.LightningModule):
    ...
    def test_mode_5(self):
        ...
        if 'desnow' in self.opt.de_type:
            self.test_Derain_Dehaze(self.desnow_set, task="desnow")
    ...
    def eval_datasets(self):
        ...
        dehaze_splits = ["Haze/"]
        dehaze_base_path = self.opt.dehaze_path
        for name in dehaze_splits:

            self.opt.dehaze_path = os.path.join(dehaze_base_path,name)
            self.dehaze_set = DerainDehazeDataset(self.opt,addnoise=False,sigma=15)
            
        deblur_splits = ["Blur/"]
        deblur_base_path = self.opt.deblur_path
        for name in deblur_splits:

            self.opt.deblur_path = os.path.join(deblur_base_path,name)
        ...
        desnow_splits = ["Snow/"]
        desnow_base_path = self.opt.desnow_path
        for name in desnow_splits:

            self.opt.desnow_path = os.path.join(desnow_base_path,name)
            self.desnow_set = DerainDehazeDataset(self.opt,addnoise=False,sigma=15)
...
def main():
    ...
    parser.add_argument('--denoise_path', type=str, default="/data/CVPR26_AiO/Val/", help='save path of test noisy images')
    parser.add_argument('--derain_path', type=str, default="/data/CVPR26_AiO/Val/", help='save path of test raining images')
    parser.add_argument('--dehaze_path', type=str, default="/data/CVPR26_AiO/Val/", help='save path of test hazy images')
    parser.add_argument('--deblur_path', type=str, default="/data/CVPR26_AiO/Val/", help='save path of test blurry images')
    parser.add_argument('--enhance_path', type=str, default="/data/CVPR26_AiO/Val/", help='save path of test low light images')
    parser.add_argument('--desnow_path', type=str, default="/data/CVPR26_AiO/Val/", help='save path of test snowy images')
    ...
    parser.add_argument('--de_type', nargs='+', default=['derain', 'dehaze', 'deblur', 'enhance', 'desnow'],
                        help='which type of degradations is training and testing for.')
    ...
    parser.add_argument('--deblur_dir', type=str, default='/data/CVPR26_AiO/Train/Blur/',
                        help='where clean images of denoising saves.')
    parser.add_argument('--enhance_dir', type=str, default='/data/CVPR26_AiO/Train/Lowlight/',
                        help='where clean images of denoising saves.')
    parser.add_argument('--derain_dir', type=str, default='/data/CVPR26_AiO/Train/Rain/',
                        help='where training images of deraining saves.')
    parser.add_argument('--dehaze_dir', type=str, default='/data/CVPR26_AiO/Train/Haze/',
                        help='where training images of dehazing saves.')
    parser.add_argument('--desnow_dir', type=str, default='/data/CVPR26_AiO/Train/Snow/',
    ...
    ckpt_path = "./checkpoints/sym-5d-149.ckpt"
    # model = BioIRModel(opt, eval_interval=1)
    model = BioIRModel.load_from_checkpoint(
    ckpt_path,
    opt=opt,
    eval_interval=1
    ).cuda()
