...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=int, default=6,
                        help='0 for denoise, 1 for derain, 2 for dehaze, 3 for deblur, 4 for enhance, 5 for all-in-one (three tasks), 6 for all-in-one (five tasks), 7 for all-in-one (five tasks including snow removal and without denoising), 8 for all-in-one (OOD test on AIO dataset)')
    
    parser.add_argument('--denoise_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test noisy images')
    parser.add_argument('--derain_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test raining images')
    parser.add_argument('--dehaze_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test hazy images')
    parser.add_argument('--deblur_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test blurry images')
    parser.add_argument('--enhance_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test low light images')
    parser.add_argument('--desnow_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test snowy images')
    parser.add_argument('--aio_path', type=str, default="/data/CVPR26_AiO/debug/", help='save path of test AIO images')
    ...
    if testopt.mode == 0:
        ...
        derain_base_path = testopt.derain_path
        for name in derain_splits:
            print('Start testing Rain...')
            testopt.gopro_path = os.path.join(derain_base_path,name)
            derain_set = DerainDehazeDataset(testopt,addnoise=False,sigma=55, task='derain')
            test_Derain_Dehaze(net, derain_set, task="derain")

        haze_base_path = testopt.dehaze_path
        for name in haze_splits:
            print('Start testing Haze...')
            testopt.dehaze_path = os.path.join(haze_base_path,name)
            derain_set = DerainDehazeDataset(testopt,addnoise=False,sigma=55, task='dehaze')
            test_Derain_Dehaze(net, derain_set, task="dehaze")

        deblur_base_path = testopt.gopro_path
        for name in deblur_splits:
            print('Start testing Blur...')
            testopt.gopro_path = os.path.join(deblur_base_path,name)
            deblur_set = DerainDehazeDataset(testopt,addnoise=False,sigma=55, task='deblur')
            test_Derain_Dehaze(net, deblur_set, task="deblur")
        ...
        desnow_base_path = testopt.desnow_path
        for name in enhance_splits:
            print('Start testing Snow...')
            testopt.desnow_path = os.path.join(desnow_base_path,name)
            derain_set = DerainDehazeDataset(testopt,addnoise=False,sigma=55, task='desnow')
            test_Derain_Dehaze(net, derain_set, task="desnow")

    elif testopt.mode == 8:
        aio_base_path = testopt.aio_path
        print('Start testing AIO...')
        testopt.aio_path = os.path.join(aio_base_path)
        print(testopt.aio_path)
        aio_set = DerainDehazeDataset(testopt,addnoise=False,sigma=55, task='aio')
        test_Derain_Dehaze(net, aio_set, task="aio")